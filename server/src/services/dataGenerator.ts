...
export function generateSyntheticDataset(): GeneratedDataset {
  const transactions: NormalizedTransaction[] = [];
  const fraudIncidents: FraudIncident[] = [];
  ...
  for (let i = 0; i < count1; i++) {
    ...
    if (receiverIdx === senderIdx) receiverIdx = (senderIdx + 1) % availableAccCount;
