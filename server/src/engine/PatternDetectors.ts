...
import { Transaction, PathNode, RetentionStage, FanOutPattern, FanInPattern, CircularFlowPattern } from '../models/types';

export class PatternDetectors {
  /**
   * Detects Fan-Out (Split transfers) from a node where 1 account disperses to multiple accounts.
   */
  public static detectFanOut(nodeId: string, graph: TransactionGraph): FanOutPattern[] {
    const patterns: FanOutPattern[] = [];
    const outEdges = graph.getOutEdges(nodeId);
    if (outEdges.length < 2) return patterns;

    // Group outgoing transactions by temporal clustering (within 60 mins)
    const sortedEdges = [...outEdges].sort((a, b) => 
      new Date(a.transaction.timestamp).getTime() - new Date(b.transaction.timestamp).getTime()
    );
   ...
   */
  public static detectFanIn(nodeId: string, graph: TransactionGraph): FanInPattern[] {
    const patterns: FanInPattern[] = [];
    const inEdges = graph.getInEdges(nodeId);
    if (inEdges.length < 2) return patterns;
   ...
   */
  public static detectCircularFlow(startNodeId: string, graph: TransactionGraph, maxDepth = 6): CircularFlowPattern[] {
    const patterns: CircularFlowPattern[] = [];
    const visited = new Set<string>();
    const currentPath: { nodeId: string; tx: Transaction }[] = [];
   ...
   */
  public static calculateFundRetention(
    nodes: PathNode[],
    ...
    const nodeInAmount = new Map<string, number>();
    const nodeOutAmount = new Map<string, number>();
   ...
   */
  public static calculateTimeIntervals(transactions: Transaction[]): {
    intervals: number[];
