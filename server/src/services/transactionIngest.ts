...
import { IDataAdapter, IngestResult } from '../adapters/IDataAdapter.js';
...
export class TransactionIngestionPipeline {
   ...
   */
  async processIncomingTransaction(rawTxn: Partial<NormalizedTransaction>): Promise<{
    result: IngestResult;
    updatedCluster?: FraudCluster;
    newlyConnectedEntities: string[];
  }> {
    ...
    const resolvedEntityIds: string[] = [senderAcc.id, receiverAcc.id];
    ...
    // Financial edge
    const finRel = this.graphService.addEdge(
      senderAcc.id, 
      receiverAcc.id, 
      'SENT_TO', 
      normalized.timestamp, 
      0.99, 
      { amount: normalized.amount, channel: normalized.channel, transactionId: normalized.transactionId }
    );
    ...
    if (normalized.deviceId) {
      ...
      const sharedDevRel = this.graphService.addEdge(
        senderAcc.id, 
        'ACCOUNT:ACC_2841_01', 
        'SHARED_DEVICE', 
        normalized.timestamp, 
        0.99, 
        { deviceId: dev.id, note: 'New account activated on syndicate device DEV_2841_ALPHA' }
      );
    ...
    const affectedCluster = clusters.find((c: FraudCluster) => 
      c.entityIds.includes(senderAcc.id) || c.entityIds.includes(receiverAcc.id)
    ) || clusters[0];
