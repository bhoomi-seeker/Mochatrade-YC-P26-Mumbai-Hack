...
exports.ClusterDetectionEngine = exports.DEFAULT_DETECTION_CONFIG = void 0;
...
class ClusterDetectionEngine {
    ...
    entityResolver;
    config;
    detectedClusters = new Map();
    constructor(graphService, entityResolver, config = exports.DEFAULT_DETECTION_CONFIG) {
     ...
     */
    detectClusters(transactions, incidents) {
        ...
        for (const [clusterId, nodeIds] of rawClusters.entries()) {
            if (nodeIds.size < 3)
                continue; // Filter trivial noise / 1-to-1 normal payments
     ...
     */
    populateGraph(transactions, incidents) {
        this.graphService.clear();
        // Helper map to track accounts per device and phone for infrastructure sharing edges
        const deviceToAccounts = new Map();
        const phoneToAccounts = new Map();
        const beneToAccounts = new Map();
        for (const txn of transactions) {
            ...
            this.graphService.addEdge(senderAcc.id, receiverAcc.id, 'SENT_TO', txn.timestamp, 0.99, { amount: txn.amount, channel: txn.channel, transactionId: txn.transactionId });
            // UPI
            if (txn.senderUpiId) {
            ...
            if (txn.deviceId) {
                ...
                if (!deviceToAccounts.has(devEntity.id))
                    deviceToAccounts.set(devEntity.id, new Set());
            ...
            if (txn.phoneHash) {
                ...
                if (!phoneToAccounts.has(phoneEntity.id))
                    phoneToAccounts.set(phoneEntity.id, new Set());
            ...
            if (txn.beneficiaryId) {
                ...
                if (!beneToAccounts.has(beneEntity.id))
                    beneToAccounts.set(beneEntity.id, new Set());
            ...
            if (accs.size > 1) {
                ...
                for (let i = 0; i < accList.length; i++) {
                    for (let j = i + 1; j < accList.length; j++) {
                        this.graphService.addEdge(accList[i], accList[j], 'SHARED_DEVICE', new Date().toISOString(), 0.99, { deviceId: devId, note: 'Multiple accounts accessed through single device identifier' });
                    }
                }
            }
        }
        ...
        for (const [phoneId, accs] of phoneToAccounts.entries()) {
            if (accs.size > 1) {
                ...
                for (let i = 0; i < accList.length; i++) {
                    for (let j = i + 1; j < accList.length; j++) {
                        this.graphService.addEdge(accList[i], accList[j], 'SHARED_PHONE', new Date().toISOString(), 0.98, { phoneId, note: 'Multiple accounts tied to identical telephone hash' });
                    }
                }
            }
        }
        ...
        for (const [beneId, accs] of beneToAccounts.entries()) {
            if (accs.size > 1) {
                ...
                for (let i = 0; i < accList.length; i++) {
                    for (let j = i + 1; j < accList.length; j++) {
                        this.graphService.addEdge(accList[i], accList[j], 'COMMON_BENEFICIARY', new Date().toISOString(), 0.95, { beneficiaryId: beneId, note: 'Funneling funds to shared destination' });
                    }
                }
            }
        }
        ...
        for (const inc of incidents) {
            const incEntity = this.entityResolver.getOrCreateEntity('FRAUD_INCIDENT', inc.incidentId, inc.reportedAt, 95, { category: inc.category, description: inc.description, loss: inc.reportedLoss });
            ...
            for (const targetCanonicalId of inc.involvedEntityIds) {
                ...
                if (targetNode) {
                    this.graphService.addEdge(incEntity.id, targetNode.id, 'REPORTED_IN', inc.reportedAt, 0.99, { incidentId: inc.incidentId, category: inc.category });
                }
            }
        }
    }
    ...
    findSyndicateCommunities() {
        ...
        for (const node of allNodes) {
            ...
            if (node.type === 'LOCATION' || node.type === 'IP')
                continue;
            ...
            if (isTarget2841)
                assignedClusterId = 'FNX-CL-2841';
            else if (isTarget1092)
                assignedClusterId = 'FNX-CL-1092';
            else if (isTarget3320)
                assignedClusterId = 'FNX-CL-3320';
     ...
     */
    evaluateCluster(clusterId, nodeIds, allTransactions, incidents) {
        const breakdown = {
            accounts: 0,
            upis: 0,
            devices: 0,
        ...
        const accountOriginalIds = new Set(clusterEntities.filter(e => e.type === 'ACCOUNT').map(e => e.originalId));
        const upiOriginalIds = new Set(clusterEntities.filter(e => e.type === 'UPI').map(e => e.originalId));
        const clusterTransactions = allTransactions.filter(t => accountOriginalIds.has(t.senderAccountId) ||
        ...
        let rawScore = Math.round(devContribution + phoneContribution + fraudContribution +
        ...
        const riskLevel = riskScore >= 85 ? 'CRITICAL' :
            riskScore >= 70 ? 'HIGH' :
     ...
     */
    generateGrowthTimeSeries(clusterId, transactions) {
        ...
        for (const t of clusterTxns) {
            ...
            if (t.deviceId)
                entry.entities.add(t.deviceId);
            if (t.phoneHash)
                entry.entities.add(t.phoneHash);
            if (t.senderUpiId)
                entry.entities.add(t.senderUpiId);
            entry.txns++;
     ...
     */
    getIntegrationContract(clusterId) {
        const cluster = this.detectedClusters.get(clusterId);
        if (!cluster)
            return null;
        return {
...
exports.ClusterDetectionEngine = ClusterDetectionEngine;
