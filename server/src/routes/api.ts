...
import { AuditLog, FraudNexusIntegrationContract, Transaction } from '../models/types';
...
export function createApiRouter(
  ...
  function logAudit(action: string, target: string, details: any) {
    auditLogs.unshift({
  ...
  router.get('/money-flow/trace', async (req: Request, res: Response) => {
    try {
      const query = (req.query.source as string) || 'TXN-FNX-9001';
      const direction = (req.query.direction as 'outgoing' | 'incoming' | 'both') || 'outgoing';
  ...
  router.get('/money-flow/path/:pathId', (req: Request, res: Response) => {
  ...
  router.get('/money-flow/:pathId/transactions', (req: Request, res: Response) => {
    const pathId = Array.isArray(req.params.pathId) ? req.params.pathId[0] : req.params.pathId;
    const path = pathCache.get(pathId);
    if (!path) {
  ...
  router.get('/money-flow/:pathId/entities', (req: Request, res: Response) => {
    const pathId = Array.isArray(req.params.pathId) ? req.params.pathId[0] : req.params.pathId;
    const path = pathCache.get(pathId);
    if (!path) {
      return res.status(404).json({ error: 'Path not found', pathId });
  ...
  router.get('/money-flow/:pathId/risk', (req: Request, res: Response) => {
    const pathId = Array.isArray(req.params.pathId) ? req.params.pathId[0] : req.params.pathId;
    const path = pathCache.get(pathId);
    if (!path) {
      return res.status(404).json({ error: 'Path not found', pathId });
  ...
  router.post('/transactions/ingest', async (req: Request, res: Response) => {
    try {
  ...
  router.post('/money-flow/recalculate', async (req: Request, res: Response) => {
    try {
  ...
  router.get('/data-source/status', (req: Request, res: Response) => {
    const status = adapter.getStatus();
    res.json({
      ...status,
      currentEnvSource: AdapterFactory.getCurrentSource(),
      supportedSources: ['demo', 'api', 'bank', 'payment', 'csv', 'webhook']
    });
  });

  // 11. POST /api/simulation/trigger (Live incoming transaction simulation)
  router.post('/simulation/trigger', async (req: Request, res: Response) => {
    try {
  ...
  router.get('/contract/export/:pathId', (req: Request, res: Response) => {
    const pathId = Array.isArray(req.params.pathId) ? req.params.pathId[0] : req.params.pathId;
    const path = pathCache.get(pathId);
    if (!path) {
      return res.status(404).json({ error: 'Path not found', pathId });
    ...
    const contract: FraudNexusIntegrationContract = {
      pathId: path.pathId,
  ...
  router.get('/quick-scenarios', (req: Request, res: Response) => {
    res.json([
      {
        id: 'scenario-primary',
