...
async function runVerificationTests() {
    ...
    const txns = await appContext_js_1.appContext.dataAdapter.getTransactions();
    const incidents = await appContext_js_1.appContext.dataAdapter.getFraudIncidents();
    const allNodes = appContext_js_1.appContext.graphService.getAllNodes();
    const allEdges = appContext_js_1.appContext.graphService.getAllEdges();
    ...
    const clusters = appContext_js_1.appContext.clusterEngine.getAllClusters();
    console.log(`- Detected Clusters count: ${clusters.length} (Requirement: at least 3) -> ${clusters.length >= 3 ? 'PASS' : 'FAIL'}`);
    const targetCluster = appContext_js_1.appContext.clusterEngine.getCluster('FNX-CL-2841');
    if (!targetCluster) {
    ...
    const contract = appContext_js_1.appContext.clusterEngine.getIntegrationContract('FNX-CL-2841');
    console.log(JSON.stringify(contract, null, 2));
    console.log('\n6. Validating Live Transaction Ingestion & Graph Update...');
    const simResult = await appContext_js_1.appContext.ingestPipeline.processIncomingTransaction({
